...
class PHPJasperXML
{
    ...
    public function connect($db_host, $db_user, $db_pass, $db_or_dsn_name, $cndriver = "mysql")
    {
        ...
        if ($cndriver == "mysql") {
            ...
            if ($this->myconn) {
                ...
                return false;
        } elseif ($cndriver == "mssql") {
            
            if (!$this->con) {
                $this->myconn = @mssql_connect($db_host, $db_user, $db_pass);
                if ($this->myconn) {
                    $seldb = @mssql_select_db($db_or_dsn_name, $this->myconn);
                    if ($seldb) {
                        $this->con = true;
                        return true;
                    } else {
            ...
            switch ($k) {
                ...
                default:
                    ...
                    foreach ($out as $b => $object) {
                        ...
                        $this->pointer[] = array(
                            "type" => "band",
                            "printWhenExpression" => $out->band->printWhenExpression . "",
    ...
    public function element_staticText($data)
    {
        ...
        $data->hyperlinkReferenceExpression = trim(str_replace(array(
            " ",
            '"'
        ), "", $data->hyperlinkReferenceExpression));
    ...
    public function element_crossTab($data)
    {
        ...
        foreach ($data->rowGroup as $r => $rd) {
            ...
            */
            ...
            $style           = array(
                "width" => $rd["width"] + 0,
                'rowheaderbgcolor' => $rowheaderbgcolor,
        ...
        */
        foreach ($data->columnGroup as $c => $cd) {
            ...
            $style         = array(
                "colheaderalign" => $colheaderalign,
                "colheadervalign" => $colheadervalign,
                "colheaderbgcolor" => $colheaderbgcolor,
                "colheaderisbold" => $colheaderisbold,
                "height" => $height
            );
        ...
        foreach ($data->crosstabCell as $ce => $cecontent) {
            ...
            $style          = array(
                "ceheaderalign" => $ceheaderalign,
                "ceheadervalign" => $ceheadervalign,
                "ceheaderbgcolor" => $ceheaderbgcolor,
                "ceheaderisbold" => $ceheaderisbold,
                "width" => $width
            );
            //   $colexpression=$d->bucket->bucketExpression;
            //  $colgroup[]=array("name"=>$c['name'],"width"=>$c['width'],"totalPosition"=>$r['totalPosition']);
    ...
    public function element_rectangle($data)
    {
        ...
        if (isset($data->graphicElement->pen["lineColor"]))
            $drawcolor = array(
                "r" => hexdec(substr($data->graphicElement->pen["lineColor"], 1, 2)),
                "g" => hexdec(substr($data->graphicElement->pen["lineColor"], 3, 2)),
                "b" => hexdec(substr($data->graphicElement->pen["lineColor"], 5, 2))
            );
    ...
    public function element_textField($data)
    {
        ...
        switch ($data->textFieldExpression) {
            case 'new java.util.Date()':
                ...
                //### End of modification                
                break;
            case '"Emitido em: "+new java.util.Date()':
                //### New: =>date("Y.m.d.",....
                $this->pointer[] = array(
                    "type" => "MultiCell",
                    "width" => $data->reportElement["width"],
                    "height" => $height,
                    "txt" => 'Emitido em: '.date("d/m/Y H:i:s"),
                    "border" => $border,
                    "align" => $align,
                    "fill" => $fill,
                    "hidden_type" => "date",
    ...
    public function element_subReport($data)
    {
        ...
        $srrepalcearr = array(
            ...
            "",
            '',
            $this->arrayParameter['SUBREPORT_DIR']
        );
    ...
    public function transferDBtoArray($host, $user, $password, $db_or_dsn_name, $cndriver = "mysql")
    {
        ...
        if ($cndriver == "odbc") {
            ...
            $result = @mssql_query($this->sql); //query from db
    ...
    public function variable_calculation($rowno)
    {
        ...
        foreach ($this->arrayVariable as $k => $out) {
            ...
            if ($out["calculation"] != "") {
                $out['target'] = str_replace(array(
                    '$F{',
                    '}'
                ), '', $out['target']); //,  (strlen($out['target'])-1) ); 
    ...
    public function outpage($out_method = "I", $filename = "")
    {
        ...
        if ($this->pdflib == "TCPDF") {
            if ($this->arrayPageSetting["orientation"] == "P")
                $this->pdf = new TCPDF($this->arrayPageSetting["orientation"], 'pt', array(
            ...
            if ($this->arrayPageSetting["orientation"] == "P")
                $this->pdf = new FPDF($this->arrayPageSetting["orientation"], 'pt', array(
                    intval($this->arrayPageSetting["pageWidth"]),
                    intval($this->arrayPageSetting["pageHeight"])
                ));
            else
    ...
    public function element_pieChart($data)
    {
        ...
        foreach ($data->categoryDataset->dataset->datasetRun->datasetParameter as $tag => $value) {
            $param[] = array(
                "$value[name]" => $value->datasetParameterExpression
            );
        }
        //          print_r($param);
    ...
    public function showLineChart($data, $y_axis)
    {
        ...
        $ylabel       = $data['ylabel'] . '';
        $xlabel       = $data['xlabel'] . '';
        $ylabel       = str_replace(array(
            '"',
            "'"
        ), '', $ylabel);
        $xlabel       = str_replace(array(
            '"',
            "'"
        ), '', $xlabel);
        ...
        $legendfontsize = 8;
        $this->chart->setFontProperties(array(
            'FontName' => "$pchartfolder/fonts/calibri.ttf",
        ...
        //else
        $ScaleSpacing = 5;
        $scalesetting = $scaleSettings = array(
            "XMargin" => 10,
            "YMargin" => 10,
            "Floating" => TRUE,
            "GridR" => 200,
        ...
        if ($Title != '') {
            ...
            $textsetting = array(
                ...
                'FontName' => "$pchartfolder/fonts/" . $titlefontname . ".ttf",
    ...
    public function showAreaChart($data, $y_axis, $type)
    {
        ...
        $this->chart->drawStackedAreaChart(array(
            "Surrounding" => 60
        ));
    ...
    public function printlongtext($fontfamily, $fontstyle, $fontsize)
    {
        ...
        if ($this->pdf->balancetext != '') {
            $this->continuenextpageText = array(
                'width' => $bltxt["width"],
                'height' => $bltxt["height"],
                'txt' => $this->pdf->balancetext,
                'border' => $bltxt["border"],
                'align' => $bltxt["align"],
                'fill' => $bltxt["fill"],
                'ln' => 1,
        ...
        if ($this->arraysqltable) {
            ...
            foreach ($this->arraysqltable as $row) {
                ...
                for ($d = 0; $d < $this->detailbandqty; $d++) {
                    ...
                    //                    ";
                    if (($checkpoint + $detailheight > $this->detailallowtill) && ($this->pdf->getPage() > 1) || ($checkpoint + $detailheight > $this->detailallowtill - $this->orititleheight) && ($this->pdf->getNumPages() == 1)) {
            ...
            echo "Não foram encontrados registros com os parâmetros informados.";
            exit(0);
        }
    ...
    public function showCrossTab($a, $y_axis)
    {
        ...
        $measurefield  = str_replace(array(
            '$F{',
            '}'
        ), "", $a['measurefield']);
        $ce            = $a['crosstabcell'];
        ...
        foreach ($arrdata as $r => $v) {
            ...
            $table .= '<tr><td style="background-color:' . $rowgroup['style']['rowheaderbgcolor'] . ';
                        text-align:' . $rowgroup['style']['rowheaderalign'] . ';' . '">' . $r . '</td>';
            foreach ($colarr as $cv) {
                if ($arrdata[$r][$cv] == "") {
                    $arrdata[$r][$cv] = 0;
                }
    ...
    public function checkoverflow($arraydata, $txt = "", $maxheight = 0)
    {
        ...
        if ($this->print_expression_result == true) {
            ...
            elseif ($arraydata["poverflow"] == "false" && $arraydata["soverflow"] == "false") {
                ...
                if ($this->pdf->balancetext != '') {
                    $this->continuenextpageText = array(
                        'width' => $arraydata["width"],
                        'height' => $arraydata["height"],
                        'txt' => $this->pdf->balancetext,
                        'border' => $arraydata["border"],
                        'align' => $arraydata["align"],
                        'fill' => $arraydata["fill"],
                        'ln' => 1,
                        'x' => $x,
                        'y' => '',
    ...
    public function formatText($txt, $pattern)
    {
        ...
        elseif ($pattern == "dd/MM/yyyy HH.mm.ss" && $txt != "")
            ...
            return mb_detect_encoding($txt, mb_detect_order(), true) === 'UTF-8' ? $txt : mb_convert_encoding($txt, 'UTF-8');;
    ...
    public function passAllArrayDatatoSubReport($PHPJasperXMLSubReport, $d, $current_y, $data)
    {
        ...
        if (isset($this->arraypageHeader)) {
            ...
            if ($this->currentband == 'pageHeader') { ///here need to add more conditions to fulfill different band subreport
                ...
                $PHPJasperXMLSubReport->TopHeightFromMainPage = $PHPJasperXMLSubReport->arrayMainPageSetting["topMargin"] + $PHPJasperXMLSubReport->arrayMainpageHeader[0]["height"] + $d['y'];
            }
            ...
            ###
            $PHPJasperXMLSubReport->BottomHeightFromMainPage       = $PHPJasperXMLSubReport->arrayMainPageSetting["bottomMargin"] + $PHPJasperXMLSubReport->arrayMainpageFooter[0]["height"];
            ...
            $PHPJasperXMLSubReport->arrayPageSetting["pageHeight"] = $this->arrayPageSetting["pageHeight"] - ($PHPJasperXMLSubReport->arrayMainPageSetting["topMargin"] + $PHPJasperXMLSubReport->arrayMainpageHeader[0]["height"] + $d['y']) - $this->arraypageFooter[0]["height"] - $PHPJasperXMLSubReport->arrayMainPageSetting["bottomMargin"] - $d['y'];
        }
        if (isset($this->arraypageFooter)) {
            $PHPJasperXMLSubReport->arrayMainpageFooter = $this->arraypageFooter;
        }
        ...
        if (sizeof($this->grouplist) > 0 && ($this->global_pointer > 0)) {
            ...
            foreach ($this->grouplist as $g) {
                ...
                if ($type == "header") {
                    ...
                    if ($this->arraysqltable[$this->global_pointer][$g['headercontent'][0]["groupExpression"]] != $this->arraysqltable[$this->global_pointer - 1][$g['headercontent'][0]["groupExpression"]]) {
