...
if($op == ""){
	...
	if (isset($_GET['dsStatusCliente'])) {
		$dsStatusCliente = strtoupper(trim($_GET['dsStatusCliente']));
	}
	
	$query = "SELECT CD_STATUS_CLIENTE,
	                 DS_STATUS_CLIENTE,
					 SN_ATIVO,
					 CD_EMPRESA	
			  FROM status_cliente
	...
	$elmnt = new StatusClienteVO();
	$elmnt->DS_STATUS_CLIENTE = $data->{'DS_STATUS_CLIENTE'};
	...
	try {	
		$svc->inserir($elmnt); 
		$elmnt->CD_STATUS_CLIENTE = $svc->getLastInsertId(); 
	...
	$data = json_decode(file_get_contents('php://input'));
	$elmnt = $svc->getVOByCD($data->{'CD_STATUS_CLIENTE'});
	$elmnt->DS_STATUS_CLIENTE = $data->{'DS_STATUS_CLIENTE'};
	$elmnt->CD_EMPRESA = $_SESSION['CD_EMPRESA'];
	$elmnt->SN_ATIVO = $data->{'SN_ATIVO'};
	...
	$db->beginTransaction();	
	try {	
		$svc->excluir($_POST['cdStatusCliente']); 
