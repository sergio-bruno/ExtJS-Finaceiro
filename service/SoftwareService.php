...
class SoftwareService extends GenericService{
	...
	function getLastInsertId(){
		...
		$db = Zend_Db_Table::getDefaultAdapter();
		$id = $db->lastInsertId('"software"');

		return $id;
	}

	function inserir(SoftwareVO $software){

		try {
			$this->tbl->insert(get_object_vars($software));
		} catch (Zend_Exception $e) {
			throw new Zend_Exception($e->getMessage());
		}
	}

	function alterar(SoftwareVO $software){

		try {
			$where = $this->quote("CD_SOFTWARE=?", $software->CD_SOFTWARE);
			$this->tbl->update(get_object_vars($software), $where);
		} catch (Zend_Exception $e) {
			throw new Zend_Exception($e->getMessage());
		}
	}
