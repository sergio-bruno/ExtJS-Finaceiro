<?php
class CargoService extends GenericService{
	...
	function getLastInsertId(){
		...
		$db = Zend_Db_Table::getDefaultAdapter();
		$id = $db->lastInsertId('"cargo"');

		return $id;
	}

	function inserir(CargoVO $cargo){
		unset($cargo->EMPRESA);

		try {
			$this->tbl->insert(get_object_vars($cargo));
		} catch (Zend_Exception $e) {
			throw new Zend_Exception($e->getMessage());
		}
	}

	function alterar(CargoVO $cargo){
		unset($cargo->EMPRESA);

		try {
			$where = $this->quote("CD_CARGO=?", $cargo->CD_CARGO);
			$this->tbl->update(get_object_vars($cargo), $where);
		} catch (Zend_Exception $e) {
			throw new Zend_Exception($e->getMessage());
		}
	}
