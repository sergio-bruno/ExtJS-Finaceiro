...
class FuncionalidadeService{
	...
	public function __construct(){
		...
		$this->init = 0;
		$this->totalCount = 0;
		$this->tbl = new FuncionalidadeTable();
	...
	function getLastInsertId(){
		...
		$db = Zend_Db_Table::getDefaultAdapter();
		$id = $db->lastInsertId('"funcionalidade"');

		return $id;
	}

	function inserir(FuncionalidadeVO $funcionalidade){

		try {
			$this->tbl->insert(get_object_vars($funcionalidade));
		} catch (Zend_Exception $e) {
			throw new Zend_Exception($e->getMessage());
		}
	}

	function alterar(FuncionalidadeVO $funcionalidade){

		try {
			$where = $this->quote("CD_FUNCIONALIDADE=?", $funcionalidade->CD_FUNCIONALIDADE);
			$this->tbl->update(get_object_vars($funcionalidade), $where);
		} catch (Zend_Exception $e) {
			throw new Zend_Exception($e->getMessage());
		}
	}
