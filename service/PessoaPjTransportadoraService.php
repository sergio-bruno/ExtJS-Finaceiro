<?php
class PessoaPjTransportadoraService extends GenericService{
	...
	public function __construct(){
		...
		$this->init = 0;
		$this->totalCount = 0;
		$this->tbl = new PessoaPjTransportadoraTable();
	...
	function getVOByRow(Zend_Db_Table_Row_Abstract $row){
		$vo = new PessoaPjTransportadoraVO();
	...
	function getLastInsertId(){
		...
		$db = Zend_Db_Table::getDefaultAdapter();
		$id = $db->lastInsertId('"pessoa_pj_transportadora"');

		return $id;
	}

	function inserir(PessoaPjTransportadoraVO $pessoa_pj_transportadora){
		unset($pessoa_pj_transportadora->PESSOA_PJ);

		try {
			$this->tbl->insert(get_object_vars($pessoa_pj_transportadora));
		} catch (Zend_Exception $e) {
			throw new Zend_Exception($e->getMessage());
		}
	}

	function alterar(PessoaPjTransportadoraVO $pessoa_pj_transportadora){
		unset($pessoa_pj_transportadora->PESSOA_PJ);

		try {
			$where = $this->quote("CD_PESSOA_PJ=?", $pessoa_pj_transportadora->CD_PESSOA_PJ);
			$this->tbl->update(get_object_vars($pessoa_pj_transportadora), $where);
		} catch (Zend_Exception $e) {
			throw new Zend_Exception($e->getMessage());
		}
	}
