...
class CfopService{
	...
	function getLastInsertId(){
		...
		$db = Zend_Db_Table::getDefaultAdapter();
		$id = $db->lastInsertId('"cfop"');

		return $id;
	}

	function inserir(CfopVO $cfop){

		try {
			$this->tbl->insert(get_object_vars($cfop));
		} catch (Zend_Exception $e) {
			throw new Zend_Exception($e->getMessage());
		}
	}

	function alterar(CfopVO $cfop){

		try {
			$where = $this->quote("CD_CFOP=?", $cfop->CD_CFOP);
			$this->tbl->update(get_object_vars($cfop), $where);
		} catch (Zend_Exception $e) {
			throw new Zend_Exception($e->getMessage());
		}
	}
