<?php
class OsLicitacaoService extends GenericService{
	...
	function getLastInsertId(){
		...
		$db = Zend_Db_Table::getDefaultAdapter();
		$id = $db->lastInsertId('"os_licitacao"');

		return $id;
	}

	function inserir(OsLicitacaoVO $os_licitacao){
		unset($os_licitacao->OS);

		try {
			$this->tbl->insert(get_object_vars($os_licitacao));
		} catch (Zend_Exception $e) {
			throw new Zend_Exception($e->getMessage());
		}
	}

	function alterar(OsLicitacaoVO $os_licitacao){
		unset($os_licitacao->OS);

		try {
			$where = $this->quote("CD_OS_LICITACAO=?", $os_licitacao->CD_OS_LICITACAO);
			$this->tbl->update(get_object_vars($os_licitacao), $where);
		} catch (Zend_Exception $e) {
			throw new Zend_Exception($e->getMessage());
		}
	}
