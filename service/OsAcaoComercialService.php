<?php
class OsAcaoComercialService extends GenericService{
	...
	public function __construct(){
		...
		$this->init = 0;
		$this->totalCount = 0;
		$this->tbl = new OsAcaoComercialTable();
	...
	function getLastInsertId(){
		...
		$db = Zend_Db_Table::getDefaultAdapter();
		$id = $db->lastInsertId('"os_acao_comercial"');

		return $id;
	}

	function inserir(OsAcaoComercialVO $os_acao_comercial){
		unset($os_acao_comercial->OS);

		try {
			$this->tbl->insert(get_object_vars($os_acao_comercial));
		} catch (Zend_Exception $e) {
			throw new Zend_Exception($e->getMessage());
		}
	}

	function alterar(OsAcaoComercialVO $os_acao_comercial){
		unset($os_acao_comercial->OS);

		try {
			$where = $this->quote("CD_OS_ACAO_COMERCIAL=?", $os_acao_comercial->CD_OS_ACAO_COMERCIAL);
			$this->tbl->update(get_object_vars($os_acao_comercial), $where);
		} catch (Zend_Exception $e) {
			throw new Zend_Exception($e->getMessage());
		}
	}
