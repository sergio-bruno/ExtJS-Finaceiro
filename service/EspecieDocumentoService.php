<?php
class EspecieDocumentoService extends GenericService{
	...
	public function __construct(){
		...
		$this->init = 0;
		$this->totalCount = 0;
		$this->tbl = new EspecieDocumentoTable();
	...
	function getLastInsertId(){
		...
		$db = Zend_Db_Table::getDefaultAdapter();
		$id = $db->lastInsertId('"especie_documento"');

		return $id;
	}

	function inserir(EspecieDocumentoVO $especie_documento){

		try {
			$this->tbl->insert(get_object_vars($especie_documento));
		} catch (Zend_Exception $e) {
			throw new Zend_Exception($e->getMessage());
		}
	}

	function alterar(EspecieDocumentoVO $especie_documento){

		try {
			$where = $this->quote("CD_ESPECIE_DOCUMENTO=?", $especie_documento->CD_ESPECIE_DOCUMENTO);
			$this->tbl->update(get_object_vars($especie_documento), $where);
		} catch (Zend_Exception $e) {
			throw new Zend_Exception($e->getMessage());
		}
	}
