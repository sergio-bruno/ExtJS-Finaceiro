<?php
class OsVisitaService extends GenericService{
	...
	function getLastInsertId(){
		...
		$db = Zend_Db_Table::getDefaultAdapter();
		$id = $db->lastInsertId('"os_visita"');

		return $id;
	}

	function inserir(OsVisitaVO $os_visita){
		unset($os_visita->OS);

		try {
			$this->tbl->insert(get_object_vars($os_visita));
		} catch (Zend_Exception $e) {
			throw new Zend_Exception($e->getMessage());
		}
	}

	function alterar(OsVisitaVO $os_visita){
		unset($os_visita->OS);

		try {
			$where = $this->quote("CD_OS_VISITA=?", $os_visita->CD_OS_VISITA);
			$this->tbl->update(get_object_vars($os_visita), $where);
		} catch (Zend_Exception $e) {
			throw new Zend_Exception($e->getMessage());
		}
	}
