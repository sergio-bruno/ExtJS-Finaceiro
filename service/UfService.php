...
class UfService{
	...
	function getLastInsertId(){
		...
		$db = Zend_Db_Table::getDefaultAdapter();
		$id = $db->lastInsertId('"uf"');

		return $id;
	}

	function inserir(UfVO $uf){
		try {
			$this->tbl->insert(get_object_vars($uf));
		} catch (Zend_Exception $e) {
			throw new Zend_Exception($e->getMessage());
		}
	}

	function alterar(UfVO $uf){
		try {
			$where = $this->quote("CD_UF=?", $uf->CD_UF);
			$this->tbl->update(get_object_vars($uf), $where);
		} catch (Zend_Exception $e) {
			throw new Zend_Exception($e->getMessage());
		}
	}
