<?php
class PessoaPjFornecedorService extends GenericService{
	...
	public function __construct(){
		...
		$this->init = 0;
		$this->totalCount = 0;
		$this->tbl = new PessoaPjFornecedorTable();
	...
	function getLastInsertId(){
		...
		$db = Zend_Db_Table::getDefaultAdapter();
		$id = $db->lastInsertId('"pessoa_pj_fornecedor"');

		return $id;
	}

	function inserir(PessoaPjFornecedorVO $pessoa_pj_fornecedor){
		unset($pessoa_pj_fornecedor->PESSOA_PJ);

		try {
			$this->tbl->insert(get_object_vars($pessoa_pj_fornecedor));
		} catch (Zend_Exception $e) {
			throw new Zend_Exception($e->getMessage());
		}
	}

	function alterar(PessoaPjFornecedorVO $pessoa_pj_fornecedor){
		unset($pessoa_pj_fornecedor->PESSOA_PJ);

		try {
			$where = $this->quote("CD_PESSOA_PJ=?", $pessoa_pj_fornecedor->CD_PESSOA_PJ);
			$this->tbl->update(get_object_vars($pessoa_pj_fornecedor), $where);
		} catch (Zend_Exception $e) {
			throw new Zend_Exception($e->getMessage());
		}
	}
