<?php
class PlanoContaService extends GenericService{
	...
	function getLastInsertId(){
		...
		$db = Zend_Db_Table::getDefaultAdapter();
		$id = $db->lastInsertId('"plano_conta"');

		return $id;
	}

	function inserir(PlanoContaVO $plano_conta){

		try {
			$this->tbl->insert(get_object_vars($plano_conta));
		} catch (Zend_Exception $e) {
			throw new Zend_Exception($e->getMessage());
		}
	}

	function alterar(PlanoContaVO $plano_conta){

		try {
			$where = $this->quote("CD_PLANO_CONTA=?", $plano_conta->CD_PLANO_CONTA);
			$this->tbl->update(get_object_vars($plano_conta), $where);
		} catch (Zend_Exception $e) {
			throw new Zend_Exception($e->getMessage());
		}
	}
