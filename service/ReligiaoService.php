...
class ReligiaoService{
	...
	function getLastInsertId(){
		...
		$db = Zend_Db_Table::getDefaultAdapter();
		$id = $db->lastInsertId('"religiao"');

		return $id;
	}

	function inserir(ReligiaoVO $religiao){

		try {
			$this->tbl->insert(get_object_vars($religiao));
		} catch (Zend_Exception $e) {
			throw new Zend_Exception($e->getMessage());
		}
	}

	function alterar(ReligiaoVO $religiao){

		try {
			$where = $this->quote("CD_RELIGIAO=?", $religiao->CD_RELIGIAO);
			$this->tbl->update(get_object_vars($religiao), $where);
		} catch (Zend_Exception $e) {
			throw new Zend_Exception($e->getMessage());
		}
	}
